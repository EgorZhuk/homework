...
export type DefaultAffairType = Array<AffairType>

// constants
const defaultAffairs: DefaultAffairType  = [ // need to fix any
...
export const filterAffairs = (affairs: AffairType[], filter: FilterType) => { // need to fix any
    return affairs.filter(el=>filter==='all' ? affairs : el.priority === filter) // need to fix
}
...
function HW2() {
    const [affairs, setAffairs] = useState<DefaultAffairType>(defaultAffairs) // need to fix any
    const [filter, setFilter] = useState<FilterType>('all')

    const filteredAffairs = filterAffairs(affairs, filter)
    const deleteAffairCallback = (_id: number) => { // need to fix any
        setAffairs(affairs.filter(el=>el._id!==_id))// need to fix
    }
